...
[![バージョン](https://img.shields.io/badge/version-1.2.0-blue.svg)](https://github.com/henrychen95/rag-flex)
...
**注意**: SDK の制限により、ドロップダウンにはプリセットモデルのみが表示されます。**カスタム埋め込みモデル**フィールドを使用して、モデルキー（例：`text-embedding-qwen3-embedding-8b`）、識別子、または完全パスを入力し、任意のダウンロード済みモデルを指定できます。
